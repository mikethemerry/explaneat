"""
ExplaNEAT Quickstart with Database Integration

This script demonstrates how to run NEAT evolution with automatic 
database tracking of experiments, populations, and genomes.
"""
import numpy as np
import torch
import neat
...
logger.info("Starting ExplaNEAT quickstart with database integration")
...
# Create database-integrated population
logger.info("Creating DatabaseBackpropPopulation...")
try:
    population = DatabaseBackpropPopulation(
        config, 
        X_train, 
        y_train,
        experiment_name="Quickstart Database Test",
        dataset_name="Random Binary Classification",
        description="Testing database integration with randomly generated data"
    )
    logger.info("DatabaseBackpropPopulation created successfully")
    ...
    experiment_summary = population.get_experiment_summary()
    logger.info(f"Experiment ID: {experiment_summary['experiment_id']}")
    logger.info(f"Experiment Name: {experiment_summary['experiment_name']}")
    
except Exception as e:
    logger.error(f"Failed to create DatabaseBackpropPopulation: {e}")
    raise

# Evolve the network
logger.info("Starting evolution process with database tracking...")
logger.info(f"Running for maximum {3} generations with {2} epochs per generation")
try:
    winner = population.run(binary_cross_entropy, n=3, nEpochs=2)
    ...
    final_summary = population.get_experiment_summary()
...
try:
    ...
    print("="*50)
    print(f"Check your PostgreSQL database 'explaneat_dev' for stored data.")
    print(f"Experiment ID: {final_summary['experiment_id']}")
    print(f"Status: {final_summary['status']}")
    print(f"Generations: {final_summary['generations_completed']}")
    print(f"Best Fitness: {final_summary['best_fitness']}")
